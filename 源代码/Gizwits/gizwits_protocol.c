...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagtempts)
    {
        info->event[info->num] = EVENT_tempts;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuetempts,(uint8_t *)&issuedData[byteFieldOffset],tempts_LEN);
        byteFieldOffset += tempts_LEN;
        dataPoints->valuetempts = gizX2Y(tempts_RATIO,  tempts_ADDITION, elongateP0FormTmp.devStatus.valuetempts); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagtds_ts)
    {
        info->event[info->num] = EVENT_tds_ts;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuetds_ts,(uint8_t *)&issuedData[byteFieldOffset],tds_ts_LEN);
        byteFieldOffset += tds_ts_LEN;
        dataPoints->valuetds_ts = gizX2Y(tds_ts_RATIO,  tds_ts_ADDITION, elongateP0FormTmp.devStatus.valuetds_ts); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagPH_L)
    {
        info->event[info->num] = EVENT_PH_L;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuePH_L,(uint8_t *)&issuedData[byteFieldOffset],PH_L_LEN);
        byteFieldOffset += PH_L_LEN;
        dataPoints->valuePH_L = gizX2Y(PH_L_RATIO,  PH_L_ADDITION, elongateP0FormTmp.devStatus.valuePH_L); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagPH_H)
    {
        info->event[info->num] = EVENT_PH_H;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuePH_H,(uint8_t *)&issuedData[byteFieldOffset],PH_H_LEN);
        byteFieldOffset += PH_H_LEN;
        dataPoints->valuePH_H = gizX2Y(PH_H_RATIO,  PH_H_ADDITION, elongateP0FormTmp.devStatus.valuePH_H); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagzhuodu_ts)
    {
        info->event[info->num] = EVENT_zhuodu_ts;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuezhuodu_ts,(uint8_t *)&issuedData[byteFieldOffset],zhuodu_ts_LEN);
        byteFieldOffset += zhuodu_ts_LEN;
        dataPoints->valuezhuodu_ts = gizX2Y(zhuodu_ts_RATIO,  zhuodu_ts_ADDITION, elongateP0FormTmp.devStatus.valuezhuodu_ts); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuetempts != cur->valuetempts)
    {
        GIZWITS_LOG("valuetempts Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagtempts = 1;
        ret = 1;
    }
    if(last->valuetds_ts != cur->valuetds_ts)
    {
        GIZWITS_LOG("valuetds_ts Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagtds_ts = 1;
        ret = 1;
    }
    if(last->valuePH_L != cur->valuePH_L)
    {
        GIZWITS_LOG("valuePH_L Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagPH_L = 1;
        ret = 1;
    }
    if(last->valuePH_H != cur->valuePH_H)
    {
        GIZWITS_LOG("valuePH_H Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagPH_H = 1;
        ret = 1;
    }
    if(last->valuezhuodu_ts != cur->valuezhuodu_ts)
    {
        GIZWITS_LOG("valuezhuodu_ts Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagzhuodu_ts = 1;
        ret = 1;
    }
    ...
    if(last->valueTDS_DAT != cur->valueTDS_DAT)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueTDS_DAT Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagTDS_DAT = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagtempts)
    {
        devStatusTmp.valuetempts = gizY2X(tempts_RATIO,  tempts_ADDITION, dataPoints->valuetempts); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuetempts,tempts_LEN);
        byteFieldOffset += tempts_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagtds_ts)
    {
        devStatusTmp.valuetds_ts = gizY2X(tds_ts_RATIO,  tds_ts_ADDITION, dataPoints->valuetds_ts); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuetds_ts,tds_ts_LEN);
        byteFieldOffset += tds_ts_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagPH_L)
    {
        devStatusTmp.valuePH_L = gizY2X(PH_L_RATIO,  PH_L_ADDITION, dataPoints->valuePH_L); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuePH_L,PH_L_LEN);
        byteFieldOffset += PH_L_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagPH_H)
    {
        devStatusTmp.valuePH_H = gizY2X(PH_H_RATIO,  PH_H_ADDITION, dataPoints->valuePH_H); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuePH_H,PH_H_LEN);
        byteFieldOffset += PH_H_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagzhuodu_ts)
    {
        devStatusTmp.valuezhuodu_ts = gizY2X(zhuodu_ts_RATIO,  zhuodu_ts_ADDITION, dataPoints->valuezhuodu_ts); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuezhuodu_ts,zhuodu_ts_LEN);
        byteFieldOffset += zhuodu_ts_LEN;
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagTDS_DAT)
    {
        devStatusTmp.valueTDS_DAT = exchangeBytes(gizY2X(TDS_DAT_RATIO,  TDS_DAT_ADDITION, dataPoints->valueTDS_DAT)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueTDS_DAT,TDS_DAT_LEN);
        byteFieldOffset += TDS_DAT_LEN;
    }
